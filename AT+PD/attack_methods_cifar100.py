...
class Attack_FeaScatter(nn.Module):
    ...
    def forward(self,
        ...
        #target_y = torch.randint(0, 10, [batch_size]).cuda()
        #target_gt = one_hot_tensor(target_y, num_classes, device)
        ...
        for i in range(iter_num):
            ...
            #ot_loss = ot.sinkhorn_loss_joint_IPOT(1, 0.00, logits_pred_nat,
                                                  ...
                                                  #0.01, m, n)
            ot_loss = self.loss_func(logits_pred, targets[indices])
            #ot_loss = self.loss_func(logits_pred, targets)
            ot_loss = ot_loss.mean()
            

            aux_net.zero_grad()
            adv_loss = ot_loss + 50 * reg
            #adv_loss.backward(retain_graph=True)
            p_all = torch.autograd.grad(adv_loss, [x], retain_graph=True)[0]
            x_adv = x.data - self.step_size * torch.sign(p_all)
            x_adv = torch.min(torch.max(x_adv, inputs - self.epsilon),
                              inputs + self.epsilon)
            x_adv = torch.clamp(x_adv, -1.0, 1.0)
            x = Variable(x_adv)

        #logits_pred_nat, fea = self.basic_net(inputs)
        logits_pred, fea = self.basic_net(x)
        self.basic_net.zero_grad()

        y_sm = utils.label_smoothing(y_gt, y_gt.size(1), self.ls_factor)
        #rand_sm = utils.label_smoothing(target_gt, target_gt.size(1), self.ls_factor)
        
        adv_loss = loss_ce(logits_pred, y_sm.detach())
        #nat_loss = loss_ce(logits_pred_nat, y_sm.detach())
        #total_loss = (adv_loss + nat_loss) / 2
